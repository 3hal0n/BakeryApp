...
router.get('/daily-sales', async (req, res) => {
  try {
    ...
    const dailySales: { [key: string]: { date: string; totalSales: number; orderCount: number; paidAmount: number; unpaidAmount: number; } } = {};
...
router.get('/popular-items', async (req, res) => {
  try {
    ...
    const itemStats: { [key: string]: { itemName: string; totalQuantity: number; totalRevenue: number; orderCount: number; } } = {};
...
router.get('/pending-orders', async (req, res) => {
  try {
    ...
    const overdue = pendingOrders.filter((o: any) => new Date(o.pickupAt) < now && o.status === 'PENDING');
    const todayOrders = pendingOrders.filter((o: any) => {
      const pickup = new Date(o.pickupAt);
      return pickup >= today && pickup < tomorrow;
    });
    const upcomingOrders = pendingOrders.filter((o: any) => new Date(o.pickupAt) >= tomorrow);

    const paymentSummary = {
      paid: pendingOrders.filter((o: any) => o.paymentStatus === 'PAID').length,
      advance: pendingOrders.filter((o: any) => o.paymentStatus === 'ADVANCE').length,
      unpaid: pendingOrders.filter((o: any) => o.paymentStatus === 'UNPAID').length,
    };

    const statusSummary = {
      pending: pendingOrders.filter((o: any) => o.status === 'PENDING').length,
      ready: pendingOrders.filter((o: any) => o.status === 'READY').length,
    };

    const totalPendingRevenue = pendingOrders.reduce((sum: number, order: any) => {
      return sum + parseFloat(order.totalAmount.toString());
    }, 0);

    const totalAdvanceReceived = pendingOrders
      .filter((o: any) => o.paymentStatus === 'ADVANCE')
      .reduce((sum: number, order: any) => sum + parseFloat(order.advanceAmount.toString()), 0);
